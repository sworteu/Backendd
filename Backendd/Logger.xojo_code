...
Inherits Thread
	...
	#tag Method, Flags = &h0
		Sub RequestLog()
		  ...
		  Dim YearFormatted As Text = CurrentDate.Year.ToText
		  Dim MonthFormatted As Text = If (CurrentDate.Month < 10, "0" + CurrentDate.Month.ToText, CurrentDate.Month.ToText)
		  Dim DayFormatted As Text = If (CurrentDate.Day < 10, "0" + CurrentDate.Day.ToText, CurrentDate.Day.ToText)
		  Dim DateFormattedT As Text = YearFormatted + MonthFormatted + DayFormatted
		  Dim DateFormatted As String = DateFormattedT.ToCString(Xojo.Core.TextEncoding.UTF8)
		  
		  // Get the current time formatted as HHMMSS.
		  Dim HourFormatted As Text = If(CurrentDate.Hour < 10, "0" + CurrentDate.Hour.ToText, CurrentDate.Hour.ToText)
		  Dim MinuteFormatted As Text = If(CurrentDate.Minute < 10, "0" + CurrentDate.Minute.ToText, CurrentDate.Minute.ToText)
		  Dim SecondFormatted As Text = If(CurrentDate.Second < 10, "0" + CurrentDate.Second.ToText, CurrentDate.Second.ToText)
		  Dim TimeFormattedT As Text = HourFormatted + ":" + MinuteFormatted + ":" +  SecondFormatted
		  Dim TimeFormatted As String = TimeFormattedT.ToCString(Xojo.Core.TextEncoding.UTF8)
